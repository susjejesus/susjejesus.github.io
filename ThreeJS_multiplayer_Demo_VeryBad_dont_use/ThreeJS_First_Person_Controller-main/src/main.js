...
// Runs when the user is logged into firebase
firebase.auth().onAuthStateChanged((user) => {
    ...
    if (user) {
        ...
        firebaseRef = firebase.database().ref(`players`);

        // Remove the user when they log out
        playerRef.onDisconnect().remove();

        // Runs the game when player is logged in
        animate();
        ...
        function animate() {
            ...
            firebaseRef.once("value", function (snapshot) {
                snapshot.forEach(function (element) {
                    if(element.val().playerId === playerId){
                        
                    } else {
                        const otherPlayer = new THREE.Mesh( new THREE.BoxGeometry(2, 2, 2), new THREE.MeshLambertMaterial({ color: 0XFFFFFF }));
                        otherPlayer.position.set( element.val().xPosition, element.val().yPosition, element.val().zPosition );
